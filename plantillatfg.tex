...
\chapter {advanced micro-controller bus architecture}
   ...
   \section {especificanción axi}
      ...
      \subsection { comunicacion en la especificacion axi }
         axi, como se ha comentado anteriormente es un protocolo de transmision
         de datos, y para ello tine definio un mecanismo de ''handshake'', para
         establecer la comunicacion entre el maestro y el esclavo.


         seguidamente, encontramos dos tipos de canales de lectura, que al igual
         que los canales de escritura son de distinito uso y tienen sus propias
         señales.  primeramente esta la tenemon el canal de direccion, donde el
         maestro emitira una señal valid cuando la direcion sea valida y se
         mantendra activada hasta que la señal ready del esclavo este activada,
         mientras que en el canal de lectura de datos sera el esclavo quien
         maneje la señal valid, que estara a uno en cuanto los datos utilizados
         por este sean validos, y se mantendra activada hasta que la señal ready
         del maestro activa.

         en cuanto, al numero de canales, axi implementa en su especificacion
         varios canales de escritura y lectura, los cuales se subdividen en
         dos mas, que son utilzados uno para el control y los registro,
         mientras que los datos canales de datos se destinan para la transmision
         de estos.

            \begin{figure}[h]
               \centering
               \includegraphics[width=10cm]{img/cap5/arquitectura_lecutra.png}
               \caption{canales de lectura}
               \label{fig:read_channel}
            \end{figure}

         como se puede apreciar en la \ref{fig:read_channel}, la interfaz
         maestra, se encarga de enviar por el canal de lectura y control, las
         diferentes señales necesaria para indicar la direccion de lecutra de
         datos y todas las señales de control para ello. seguidamente el
         esclavo, se encargara de enviar los datos requeridos mediante las
         señales recibidas, esto se llevara a cargo por el canal de lectura de
         datos, el cual se dedica a la transmision de los datos pedidos.

            \begin{figure}[h]
               \centering
               \includegraphics[width=10cm]{img/cap5/arquitectura_escritura.png}
               \caption{canales de escritura}
               \label{fig:write_channels}
            \end{figure}
         
         en cambio, en los canales de escritura, la interfaz esclava solamente
         es receptora, a diferencia que en los canales de lectura y solamente
         emitira respuesta del estado de la escritura. como se ve en la
         \ref{fig:write_channels}, esta primeramente utiliza el canal de
         registros para enviar las señales necesaria para indicar la direccion
         para realizar la escritura y las señales de control. donde seguidamente
         se utilizar el canal de ecritura de datos, para realizar la
         transferencia de datos. por ultimo, para saber si todo a ido correcto o
         ha habido algun fallo en el esclavo en el momento de escritura, este
         utiliza un canal emisor, ''write response'', para indicar el estado.
         
         no obstante, en la especificacion axi, a diferencia de el axi4 y
         axi4-lite, que ejecutaria las acciones de escritura y lectura de las
         formas vista anteriromente, la interfaz axi-stream, solamente realiza
         escritura de datos en rafaga. para ello, ambas partes deberan de tener
         la interfaz adecuada, para asi poder transmitir de maestro a esclavo
         por un canal unidercional los datos necesarios.

      \subsection{señales disponibles}
          
         \begin{table}[h]
            \begin{center}
                  \begin{tabular}{|l|c|c|}
                  \hline
                  descripcion de la señal & escritura  &  lectura \\ \hline
                  id, señal utilizada para la identificacion de multiples transmisiones en un solo canal    &  x  &     \\ \hline
                  flexibilidad   &     &  x  \\ \hline
                  aprendizaje    &     &  x  \\
                  \hline
               \end{tabular}
               \caption{conclusion lenguajes hdl vs hls}
               \label{tab:signal_desc}
            \end{center}
         \end{table}

      \subsection{ Handshake }

         este mecanismo se basa en el uso de dos señales, valid, la cual indica
         si las señales de direcion/control y los datos son validos para la
         fuente de transmision, mientras que, la señal ready, es utilizada por
         el destinatario de los datos, para señalar que puede aceptar mas
         informacion. este mecansimo se encuentra en cada uno de los canales
         donde se transmite informacion, siendo estas señales propias de cada
         canal.

         estas señales son utilizadas de distinta forma dependiendo del tipo de
         canal, en el caso de los canales de escritura, encontramos tres tipos
         de canales, el canal de direccion/control, donde la señal valid sera
         uno cuando la direccion que maneja el maestro sea valida, una vez esta
         señal este activada el maestro tendra que mantener esta señal valid
         activada hasta que la señal ready del esclavo este activada. por otra
         parte, encontramos el canal de escritura de datos, donde el maestro
         solamente tendra activada la señal vallid cuando los datos que
         proporciona son validos, después al igual que en el canal mencionado
         anteriormente, este esperara a que la señal ready del esclavo este
         activada. por ultimo, encontramos el canal encargado de dar una
         respuesta por parte del esclavo cuando la transferencia es finalizada.
         este canal, el esclavo puede activar la señal de validacion, cuando
         este haya generado una respuesta valida y tendra que mantenerse
         activada hasta que el maestro tenga la señal de ready activada.

      \subsection{ Operaciones de escritura y lectura }

         Vista las señales disponibles en las interfaces AXI y el esquema del
         proceso, ya podemos profundizar en las operaciones disponibles, siendo
         estas, escritura y lectura. Primeramente, en el proceso de escritura,
         como hemos visto anteriormente, el maestro debe proporcionar las
         señales necesaria en su distintos canales de transmision, y finalmente
         este recibira el estado de la transmision a traves del canal de
         respuesta, el cual tiene las señales proporcionadas por el receptor de
         los datos.

            \begin{figure}[h]
               \centering
               \includegraphics[width=10cm]{img/cap5/500px-axi_write_transaction.svg.png}
               \caption{proceso de escritura}
               \label{fig:write_transaction}
            \end{figure}

         El proceso inicia en el canal de direccion/control, donde las señales
         AWADDR, proporciona la direccion de escritura, ARSIZE, indicando el
         tamaño de los datos, ARLEN, el tamaño de la rafaga y AWBURST como el
         tipo de rafaga a utilizar. Seguidamente con las señales de control y
         direccion sean validas, empiezara el proceso de ''handshake''. Después,
         empezara la transmision de los datos a traves de WDATA, en el canal de
         datos. Una vez terminado, se puede ver en \ref{fig:write_transaction}
         que el esclavo utilizara BRESP para señalar cual es el estado del
         proceso en el canal de respuesta.

         En cuanto a la operacion de lectura, Como se puede ver en
         \ref{fig:read_transaction}, que este solamente, necesita de dos canales
         para la lectura de datos.

            \begin{figure}[h]
               \centering
               \includegraphics[width=10cm]{img/cap5/1024px-AXI_read_transaction.svg.png}
               \caption{proceso de escritura}
               \label{fig:read_transaction}
            \end{figure}

         El proceso de lectura, el cual no difiere mucho del anterior operacion,
         no necesita de un canal de respuesta por parte del esclavo. Por tanto,
         por parte del canal de lectura de direcion/control, sera igual que en
         el canal de escritura, mientras que en el canal de lectura de datos,
         sera el maestro quien generara la señal del estatus del proceso o
         RRESP.

      \subsection{ Rafaga en la especificación AXI }

      La especificacion AXI, esta basada en un protocolo de rafaga de datos, lo
      que proporciona la capacidad de hacer multiples transferencia en una sola
      solicitud, una utildad muy util en caso de transmitir una gran cantidad de
      datos.

      Actualmente, la especificacion cuenta con tres tipos de rafagas que pueden
      ser selecionadas mediante la señal AWBURST o ARBURST. En primer lugar,
      tenemos la opcion FIXED, la cual transmite la misma direcion en cada
      ''beat''. Esta, opcione es interesante en el caso de acceder multiples
      veces a la misma direccion de memoria. Por otra parte, encontramos la
      opcion INCR, la cual es util para la lectura y escritura de datos en
      partes colindantes en memoria.

         \begin{center}
            \begin{equation}
               \mathbf{Direcci\acute{o}n_i = Direcci\acute{o}nInicial + i \cdot Tama \tilde{n}oTransferencia}
            \end{equation}
         \end{center}

      Esta se basa en la direccion inicial, donde devolvera la direccion
      dependiendo del tamaño del archivo y los ''beats'' transcurridos.
      Finalmente, como ultima opcion esta WRAP, una opcion similar a la
      anterior, no obstante, cuando esta exede el limite, se restablece la
      direccion a la la direccion anterior.
        
            \begin{equation}
               \begin{split}
                  \mathbf{Direcci\acute{o}n_i = Limite + (Direcci\acute{o}nInicial + i \cdot Tama \tilde{n}oTransferencia)} \\
                  \mathbf{mod (TamañoBurst \cdot Tama \tilde{n}oTransferencia)}
               \end{split}
            \end{equation}
      ...
      Donde, 
         ...
         \begin{center}
            \begin{equation}
               \begin{split}
                  \mathbf{Limite =  \left\lfloor\dfrac{Direcci\acute{o}nInicial}{NumeroBytes \cdot TamañoBurst}\right\rfloor  \cdot (NumeroBytes \cdot TamañoBurst)}
               \end{split}
            \end{equation}
         \end{center}
